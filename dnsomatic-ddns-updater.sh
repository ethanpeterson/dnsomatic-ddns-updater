...
then
  ...
  # if the dig command exists then use it to find our old ip address, otherwise fallback on nslookup
  if [ ! -z "$digExists" ]
  then
    old_ip=$(dig $record_name +short)
  else
    old_ip=$(nslookup $record_name | grep -o -E '([0-9][0-9]?[0-9]?\.?){4}$' | awk 'NR==2')
  fi
...
if [ "${old_ip}" == "" ]
then 
  echo "Requesting to update all DNS records at DNS-O-Matic"
